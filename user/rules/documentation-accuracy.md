...
- **Verify against source before writing a line.** Read the real implementation — routes,
  handlers, schemas, models, configuration, jobs — and know the `file:line` each claim comes
...
- **Exercise the real path where a claim is testable.** Real request and response, real route
  resolution, real input validation, real command invocation. Never write a passing
...
- **Copy, don't paraphrase, the specifics.** Field lists, defaults, limits, enums, and
  error cases come from the code — schema definitions, configuration constants, handler
  logic — not from a plausible reconstruction. Example payloads use real field names and real value
